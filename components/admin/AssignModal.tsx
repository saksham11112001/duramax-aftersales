...
export default function AssignModal({ ticket, supervisors, assignType, onConfirm, onClose, loading }: Props) {
  const today   = new Date().toISOString().split('T')[0]
  const field   = supervisors.filter(s => s.role === assignType || s.role === 'supervisor' || s.role === 'installer')
  const [selId, setSelId] = useState(field[0]?.id ?? '')
  const [date,  setDate]  = useState(today)
  const [slot,  setSlot]  = useState(SLOTS[0])
  const [notes, setNotes] = useState('')

  const isSup = assignType === 'supervisor'
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(15,15,12,.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:500, padding:16, backdropFilter:'blur(4px)' }} onClick={onClose}>
      <div style={{ background:'white', borderRadius:18, width:'100%', maxWidth:440, boxShadow:'var(--sh-lg)', overflow:'hidden', animation:'sU .26s ease' }} onClick={e=>e.stopPropagation()}>
        <div style={{ background:'linear-gradient(135deg,var(--teal),var(--teal-m))', padding:'18px 22px' }}>
          <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,.55)', textTransform:'uppercase', letterSpacing:'.08em', marginBottom:4 }}>
            {isSup ? 'Assign Supervisor' : 'Assign Installer'}
          </div>
        ...
        <div style={{ padding:'18px 22px', display:'flex', flexDirection:'column', gap:13 }}>
          <div>
            <label style={lS}>Select {isSup ? 'Supervisor' : 'Installer'}</label>
            {field.length === 0 ? (
              <div style={{ background:'var(--coral-l)', border:'1px solid #FABDB0', color:'var(--coral)', fontSize:13, padding:'11px 13px', borderRadius:8 }}>
                No {assignType}s found. <a href="/dashboard/staff" style={{ color:'var(--coral)', fontWeight:700 }}>Add staff →</a>
              </div>
            ) : (
              <select style={{ ...iS, cursor:'pointer' }} value={selId} onChange={e=>setSelId(e.target.value)} onFocus={e=>{e.target.style.borderColor='var(--teal)';e.target.style.background='#fff'}} onBlur={e=>{e.target.style.borderColor='var(--border)';e.target.style.background='var(--bg)'}}>
                {field.map(s=><option key={s.id} value={s.id}>{s.role==='installer'?'🛠️':'🔧'} {s.full_name}{s.mobile?` · ${s.mobile}`:''}</option>)}
              </select>
            )}
          </div>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:11 }}>
            <div><label style={lS}>{isSup?'Visit':'Schedule'} Date</label><input type="date" style={iS} value={date} min={today} onChange={e=>setDate(e.target.value)} onFocus={e=>e.target.style.borderColor='var(--teal)'} onBlur={e=>e.target.style.borderColor='var(--border)'}/></div>
          ...
          <div><label style={lS}>Notes {isSup?'for Supervisor':'& Parts List'} (optional)</label><textarea style={{ ...iS, resize:'none', minHeight:60 }} placeholder={isSup?"Access instructions, landmark…":"Approved parts list, special instructions…"} value={notes} onChange={e=>setNotes(e.target.value)}/></div>

          {/* Auto-reminder note — v5 feature */}
          <div style={{ background:'#EEF4FF', border:'1px solid #B5D4F4', borderRadius:9, padding:'10px 13px', display:'flex', gap:9 }}>
            ...
            <div style={{ fontSize:12, color:'#0C447C', lineHeight:1.5 }}>
              ...
              {isSup ? 'Supervisor receives WhatsApp with site details, client complaint, visit time & location as soon as you assign.' : 'Installer receives WhatsApp with client address, approved parts list & visit schedule as soon as you assign.'}
            </div>
          </div>
        </div>
